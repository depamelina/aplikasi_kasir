					...
					<table id="example1" class="table table-bordered table-striped">
					  ...
					  <tr>
    					...
    					<td>
											 ...
											 href="<?= base_url()?>Kasir/CetakFakturPenjualan/<?= $o->id ?>"
											 target="_blank">
...
<!--Modal-->
	...
	<script>
		...
		function save(){
			...
			$.ajax({
				url		: "<?= base_url()?>Kasir/SaveTransaksiPenjualan",
				method	: "POST",
				data	: {
				...
				success	: function(data){
					window.location = "<?= base_url()?>Kasir/penjualan";
				}
			});
		}
		...
		function tambah() {
			...
			function hapuss(){
				var id = $('[name="id"]').val();
				$.ajax({
					url		: "<?= base_url()?>Kasir/penjualan_delete",
					method	: "POST",
					data	: {id: id},
					success	: function(data){
						window.location = "<?= base_url()?>Kasir/penjualan";
