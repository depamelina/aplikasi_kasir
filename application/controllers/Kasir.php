...
public function __construct()
	...
	public function index()
	{
		...
		$pembelian = $this->m->Get_All('dt_penjualan', $select);
		$data['pembelian']=0;
		foreach($pembelian as $p) {
			$data['pembelian']=$p->total_beli;
		}
		$select = $this->db->select('sum(harga_jual) as total_jual, sum(harga_beli) as total_beli');
		$select = $this->db->join('dt_penjualan', 'ht_penjualan.id=dt_penjualan.id');
		$laba = $this->m->Get_All('ht_penjualan', $select);
		$data['laba']=0;
		foreach($laba as $p) {
			$data['laba']=$p->total_jual - $p->total_beli;
		}
		$this->load->view('kasir/index',$data);
	}

	public function error(){
		$this->load->view('kasir/error');
	}
	...
	public function supplier()
	{
		...
		$this->load->view('kasir/supplier',$data);
	}
	...
	public function karyawan()
	{
		...
		$this->load->view('kasir/karyawan',$data);
	}
	...
	public function LaporanpenjualanRE()
	{
		...
		$this->load->view('kasir/laporan-penjualan-re',$data);
	}
	...
	public function CetakLaporanPenjualanRE(){
		...
		$this->load->view('kasir/cetak-laporan-penjualan-re',$data);
	}
	...
	public function LaporanpenjualanOwner()
	{
		...
		$data['read_owner'] = $this->m->Get_All('owner',$select);
		$this->load->view('kasir/laporan-penjualan-owner',$data,$getowner);
	}
	...
	public function CetakLaporanPenjualanOwner(){
		...
		$this->load->view('kasir/cetak-laporan-penjualan-owner',$data);
	}
	...
	public function LaporanProduk(){
		...
		$this->load->view('kasir/laporan-produk',$data);
	}
	...
	public function CetakLaporanProduk(){
		...
		$select = $this->db->group_by('dt_penjualan.id_produk');
		$data['read_produk'] = $this->m->Get_All('produk',$select);
		$this->load->view('kasir/cetak-laporan-produk',$data);
	}

	public function LaporanPiutang(){
		$data['dari'] = date('Y-m-d', strtotime(date('Y-m-d') . ' - 1 months'));
		$data['sampai'] = date('Y-m-d');
		$this->load->view('kasir/laporan-piutang', $data);
	}
	...
	public function CetakLaporanPiutang(){
		...
		$this->load->view('kasir/cetak-laporan-piutang', $data);
	}
	...
	public function Update3()
	{
		...
		redirect(base_url().'kasir/karyawan');
	}
	...
	public function Update4()
	{
		...
		redirect(base_url().'kasir/supplier');
	}
	...
	public function Update5()
	{
		...
		redirect(base_url().'kasir/kategori');
	}
	...
	public function Update6()
	{
		...
		redirect(base_url().'kasir/customer');
	}
	...
	public function getProduk2()
	{
		...
		$this->load->view('kasir/getProduk2', $data);
	}
	...
	public function LaporanPembelian()
	{
		$data['dari'] = date('Y-m-d');
		$data['sampai'] = date('Y-m-d', strtotime($data['dari']. ' + 1 months'));
		$this->load->view('kasir/laporan-pembelian',$data);
	}
	...
	public function CetakLaporanPembelian(){
		...
		$data['dari'] = $_GET['dari'];
		$data['sampai'] = $_GET['sampai'];
		$this->load->view('kasir/cetak-laporan-pembelian',$data);
	}
	...
	public function LaporanPembelianSupplier()
	{
		...
		$select = $this->db->select('*');
		$data['read_supplier'] = $this->m->Get_All('supplier',$select);
		$this->load->view('kasir/laporan-pembelian-supplier',$data,$getsupp);
	}
	...
	public function CetakLaporanPembelianSupplier(){
		...
		$this->load->view('kasir/cetak-laporan-pembelian-supplier',$data);
	}
	...
	public function LaporanPemasukan()
	{
		$data['dari'] = date('Y-m-d');
		$data['sampai'] = date('Y-m-d', strtotime($data['dari']. ' + 1 months'));
		$this->load->view('kasir/laporan-pemasukan',$data);
	}
	
	public function CetakLaporanPemasukan(){
		$select = $this->db->select('*');
		$data['read_produk'] = $this->m->Get_All('produk',$select);		
		$this->load->view('kasir/cetak-laporan-pemasukan',$data);
	}
	...
	public function LaporanPengeluaran()
	{
		$data['dari'] = date('Y-m-d');
		$data['sampai'] = date('Y-m-d', strtotime($data['dari']. ' + 1 months'));
		$this->load->view('kasir/laporan-pengeluaran',$data);
	}

	public function CetakLaporanPengeluaran(){
		$select = $this->db->select('*');
		$data['read_produk'] = $this->m->Get_All('produk',$select);		
		$this->load->view('kasir/cetak-laporan-pengeluaran',$data);
	}
